...
export type RecentFileItem = { kind: FileKind; name: string; meta: string };

type FilesSidebarProps = {
  usageLabel: string;
  usagePercent: number;
  recentItems: RecentFileItem[];
};
 ...
 * TASK-029: 저장 용량/최근 활동을 실제 files 테이블에서 계산한다. 용량 상한이
 * 어디에도 정의돼 있지 않아(AGENTS.md 11.8절도 "고려한다" 수준) usagePercent는
 * page.tsx가 5GB를 임시 기준으로 계산한 값이다 — 실제 정책이 정해지면 바꿔야 한다.
 */
export function FilesSidebar({ usageLabel, usagePercent, recentItems }: FilesSidebarProps) {
  return (
    <div>
      <div className="mb-4 rounded-panel border border-border bg-bg-panel px-[18px] pt-[18px] pb-4">
        ...
        <div className="font-mono text-[11px] text-silk-faint">{usageLabel} 사용 중</div>
