...
/**
 * ScheduleSidebar의 "팀 일정" 카드. PersonalEventCard와 동일한 요일별 그룹 +
...
export function TeamEventCard({ items }: { items: PersonalEventItem[] }) {
  ...
  return (
    <div className="mb-[14px] rounded-panel border border-border bg-bg-panel px-4 pt-[15px] pb-[14px]">
      <p className="m-0 mb-2.5 text-[13px] font-semibold">팀 일정</p>

      {groups.length === 0 && <p className="m-0 py-1.5 text-[11.5px] text-silk-faint">등록된 팀 일정이 없습니다.</p>}
      ...
      <EditPersonalEventModal event={editing} heading="팀 일정 수정" onClose={() => setEditing(null)} />
    </div>
  );
}
