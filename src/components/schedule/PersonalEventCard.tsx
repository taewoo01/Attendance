...
/**
 * ScheduleSidebar의 "내 일정" 카드 — 본인 소유 personal_events 전체를 목록으로
 ...
 * 동일한 리스트 + × 버튼 패턴).
 * 내 일정도 계속 쌓이면 카드가 한없이 길어져서, FixedScheduleCard와 동일하게
...
export function PersonalEventCard({ items }: { items: PersonalEventItem[] }) {
  ...
  return (
    <div className="mb-[14px] rounded-panel border border-border bg-bg-panel px-4 pt-[15px] pb-[14px]">
      <p className="m-0 mb-2.5 text-[13px] font-semibold">내 일정</p>

      {groups.length === 0 && (
        <p className="m-0 py-1.5 text-[11.5px] text-silk-faint">등록된 내 일정이 없습니다.</p>
      )}
