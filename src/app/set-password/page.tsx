 ...
 * src/app/auth/callback이 만든 세션이 있을 때만 실질적으로 도달 가능하다.
 * 로그인 관련 온보딩 gap: 이 라우트는 초대 수락(invite)과 비밀번호 재설정(recovery)
 * 을 구분하지 않고 공유한다(auth/callback이 보안상 type을 구분해서 넘기지 않는다)
 * — 대신 profiles.name이 비어있으면(=이름을 한 번도 입력한 적 없는 초대 수락
 * 최초 로그인) 이름 입력칸을 같이 보여준다.
 */
